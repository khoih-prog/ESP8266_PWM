...
class ESP8266TimerInterrupt
{
  ...
  public:
    ...
    bool setFrequency(float frequency, esp8266_timer_callback callback)
    {
      bool isOKFlag = true;
      ...
      // count up
      PWM_LOGWARN3(F("ESP8266TimerInterrupt: _fre ="), _frequency, F(", _count ="), _timerCount);
    ...
    // No params and duration now. To be addes in the future by adding similar functions here or to esp32-hal-timer.c
    bool setInterval(unsigned long interval, esp8266_timer_callback callback)
    {
      return setFrequency((float) (1000000.0f / interval), callback);
    }

    bool attachInterrupt(float frequency, esp8266_timer_callback callback)
    {
    ...
    // interval (in microseconds) and duration (in milliseconds). Duration = 0 or not specified => run indefinitely
    // No params and duration now. To be addes in the future by adding similar functions here or to esp32-hal-timer.c
    bool attachInterruptInterval(unsigned long interval, esp8266_timer_callback callback)
    {
      return setFrequency( (float) ( 1000000.0f / interval), callback);
    }
